      ...
      <main className="container mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-2 text-gray-400 backdrop-blur-sm bg-[#1A0505]/60 rounded-lg px-3 py-2 border border-red-900/10">
            <Button variant="ghost" className="hover:text-white p-0" onClick={() => router.back()}>
        ...
        <div className="grid gap-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="bg-[#1A0808]/70 backdrop-blur-sm rounded-xl p-6 border border-red-900/20 shadow-lg">
              ...
              <div className="text-3xl font-bold">${(prizeData.totalDeposited / 1000000).toFixed(1)}M</div>
              <div className="text-sm text-gray-400 mt-1">${prizeData.tvl.toLocaleString()} in TVL</div>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 bg-[#1A0808]/70 backdrop-blur-sm rounded-xl p-6 border border-red-900/20 shadow-lg">
              <h3 className="text-lg font-medium mb-6">Prize Pool Contributions</h3>
              <div className="h-64 flex items-end justify-between gap-2">
                {prizeData.chartData.map((value, i) => (
                  <div key={i} className="w-full">
                    <div className="bg-red-600 rounded-t-sm" style={{ height: `${value}%` }}></div>
                  ...
                  {prizeData.prizes.map((prize, i) => (
                    <div key={i} className="flex justify-between">
                      <span
                        ...
                        style={{ fontSize: i === 0 ? "1.25rem" : i < 3 ? "1.125rem" : "1rem" }}
                      >
                        ${prize.amount.toLocaleString(undefined, { minimumFractionDigits: prize.amount < 1 ? 2 : 0 })}
                      </span>
