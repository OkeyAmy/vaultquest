...
export default function VaultPage() {
      ...
      prevVaults.map((vault) => {
        if (vault.id === vaultId) {
          return {
            ...
            deposits: [
              { address, amount: Number.parseFloat(amount), date: new Date().toISOString().split("T")[0] },
              ...vault.deposits,
            ],
          }
        }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#1A0505] to-[#2D0A0A] text-white">
                  ...
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <div className="text-sm text-gray-400 capitalize">{vault.network}</div>
                      </div>
                    </div>
                    <div className="bg-green-900/20 text-green-500 px-2 py-1 rounded text-sm">{vault.apy}% APY</div>
                  </div>
                  ...
                  <div className="space-y-3 mb-4">
                    <div className="flex justify-between items-center">
                      ...
                      <div>
                        ...
                        <div className="text-xs text-gray-400 text-right">{vault.tvlToken}</div>
                      </div>
                    </div>
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <div>
                        ...
                        <div className="text-xs text-gray-400 text-right">{vault.balanceToken}</div>
                      </div>
                    </div>
                  </div>
                      ...
                      <tbody>
                        {filteredVaults.flatMap((vault) =>
                          vault.deposits.slice(0, 3).map((deposit, i) => (
                            <tr key={`${vault.id}-${i}`} className="border-b border-red-900/10">
                              <td className="py-4">
                                <div className="flex items-center gap-2">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="bg-[#1A0808]/70 backdrop-blur-sm rounded-xl p-6 border border-red-900/20">
                      ...
                      <div className="text-3xl font-bold">
                        ${filteredVaults.reduce((sum, vault) => sum + vault.tvl, 0).toLocaleString()}
                      </div>
                      <div className="mt-2 text-sm text-gray-400">Across {filteredVaults.length} vaults</div>
                    </div>

                    <div className="bg-[#1A0808]/70 backdrop-blur-sm rounded-xl p-6 border border-red-900/20">
                      <h3 className="text-lg font-bold mb-4">Total Users</h3>
                      <div className="text-3xl font-bold">
                        {filteredVaults.reduce((sum, vault) => sum + vault.users, 0).toLocaleString()}
                      </div>
                      <div className="mt-2 text-sm text-gray-400">Active depositors</div>
                    </div>

                    <div className="bg-[#1A0808]/70 backdrop-blur-sm rounded-xl p-6 border border-red-900/20">
                      <h3 className="text-lg font-bold mb-4">Average APY</h3>
                      <div className="text-3xl font-bold">
                        {(filteredVaults.reduce((sum, vault) => sum + vault.apy, 0) / filteredVaults.length).toFixed(2)}
                        %
                      </div>
                      <div className="mt-2 text-sm text-gray-400">Weighted average</div>
                    </div>
                  </div>
                </TabsContent>
              </Tabs>
      ...
      <CreateVaultModal isOpen={isCreateModalOpen} onClose={() => setIsCreateModalOpen(false)} />
