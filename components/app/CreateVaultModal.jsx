...
export default function CreateVaultModal({ isOpen, onClose, onCreateVault }) {
  ...
  const handleCreate = () => {
    // Validate form
    if (!formData.name || !formData.token || !formData.network || !formData.yieldSource || !formData.drawDuration) {
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="bg-[#1A0808]/90 backdrop-blur-sm border border-red-900/20 text-white sm:max-w-[425px] shadow-lg">
        ...
        <div className="py-4 space-y-4">
          <div>
            <label className="text-sm text-gray-400 mb-1 block">Vault Name</label>
            <Input
          ...
          <div>
            <label className="text-sm text-gray-400 mb-1 block">Yield Source</label>
            <Select
          ...
          <div>
            <label className="text-sm text-gray-400 mb-1 block">Draw Duration</label>
            <Select
        ...
        {!isConnected ? (
          <Button className="w-full bg-red-600 hover:bg-red-700" onClick={handleConnect}>
            Connect Wallet
          </Button>
        ) : (
          <Button
